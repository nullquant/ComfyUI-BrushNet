...
class BrushNetCAModel(ModelMixin, ConfigMixin):
    """
    A BrushNetCA model.
    ...
    def from_unet(
        ...
        r"""
        Instantiate a [`BrushNetCAModel`] from [`UNet2DConditionModel`].

        Parameters:
            unet (`UNet2DConditionModel`):
                The UNet model weights to copy to the [`BrushNetCAModel`]. All configuration options are also copied
                where applicable.
        """
    ...
    def forward(
        ...
        """
        The [`BrushNetCAModel`] forward method.
